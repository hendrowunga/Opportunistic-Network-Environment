...
public class DecisionEngineRouter extends ActiveRouter {
     ...
     protected int startTransfer(Message m, Connection con) {
          ...
          } else if (tombstoning && retVal == MessageRouter.DENIED_DELIVERED) {
               this.deleteMessage(m.getId(), false);
               tombstones.add(m.getId());
          } else if (deleteDelivered && (retVal == DENIED_OLD || retVal == MessageRouter.DENIED_DELIVERED) &&
     ...
     public int receiveMessage(Message m, DTNHost from) {
          if (isDeliveredMessage(m) || (tombstoning && tombstones.contains(m.getId())))
               return MessageRouter.DENIED_DELIVERED;
